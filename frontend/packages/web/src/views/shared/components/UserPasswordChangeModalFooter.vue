...
function validateForm() {
    const errors = {};
    const oldPassword = normalizeText(props.context.oldPassword);
...
async function handleSubmit() {
    ...
    props.context.submitError = '';
    try {
        const encryptedOldPassword = await encryptPassword(
            normalizeText(props.context.oldPassword)
        );
        const encryptedNewPassword = await encryptPassword(normalizeText(props.context.password));
        ...
        props.context.submitError = error?.message || '修改密码失败';
    } finally {
